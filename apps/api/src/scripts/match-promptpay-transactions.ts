...
async function main() {
  ...
  if (!options.filePath) {
    throw new Error("กรุณาระบุ --file <path-to-json>");
  }

  const raw = await fs.readFile(options.filePath, "utf8");
  const parsed = JSON.parse(raw.replace(/^\uFEFF/, "")) as unknown;
  if (!Array.isArray(parsed)) {
    throw new Error("ไฟล์ JSON ต้องเป็น array ของรายการธุรกรรม");
  }

  const response = await fetch(`${options.baseUrl}/api/internal/promptpay/match-transactions`, {
    method: "POST",
    headers: {
