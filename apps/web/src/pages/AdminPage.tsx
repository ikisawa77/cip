  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        <section className="panel rounded-[2.5rem] p-6">
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-2">
            {providerQuery.data?.map((provider) => {
              ...
              return (
                <button
                  className={`rounded-[1.5rem] border px-4 py-4 text-left transition ${isSelected ? "border-[var(--text)] bg-[var(--text)] text-white" : "border-[var(--line)] bg-white/88 text-slate-900"}`}
                  key={provider.id}
        ...
        <section className="panel rounded-[2.5rem] p-6">
          <p className="section-label">Inventory Import</p>
          <h2 className="mt-2 text-2xl font-semibold text-slate-950">นำเข้าสินค้าพร้อมส่ง</h2>
          <p className="mt-2 text-sm muted-text">ใส่ `productId` และวางข้อมูลทีละบรรทัดสำหรับ code, download link หรือ account</p>
          <div className="mt-5 space-y-3">
            <input className="input-field" onChange={(event) => setInventoryProductId(event.target.value)} placeholder="productId" value={inventoryProductId} />
            <textarea
            ...
            />
            <button className="primary-button rounded-full px-4 py-3 text-sm" onClick={() => void inventoryMutation.mutate()} type="button">
              import inventory
            </button>
      ...
      <section className="panel rounded-[2.5rem] p-6">
        <p className="section-label">Inventory Summary</p>
